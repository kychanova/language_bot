...
from utils.handler_utils import make_word_train_text, make_word_train_task
...
async def user_word_input_handler(message: types.Message, state: FSMContext):
    ...
    if message.text == target_word_list['word']:
        ...
        # elif state_data['attempts'] > 0:
        #     text = make_word_train_text(target_word_list)
        #     if text == '':
        #         text = "Try one more time!"
        #     else:
        #         text = '<strong>Additional info:</strong>\n' + text
        #     await state.update_data(target_word_list=target_word_list, attempts=state_data['attempts']+1)
        else:
            text = "Try one more time!"
